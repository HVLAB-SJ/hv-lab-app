/**
 * 대용량 스펙북 아이템 마이그레이션 (4개 초과 아이템)
 * - 모든 base64 이미지를 Storage로 업로드
 * - Firestore에 URL로 저장
 */
...
async function processItem(accessToken, itemId) {
  ...
  if (detail.image_url && isBase64Image(detail.image_url)) {
    const path = `specbook_items/${itemId}/main.jpg`;
    try {
  ...
  if (Array.isArray(detail.sub_images) && detail.sub_images.length > 0) {
    ...
    for (let i = 0; i < detail.sub_images.length; i++) {
      ...
      if (isBase64Image(img)) {
        const path = `specbook_items/${itemId}/sub_${i}.jpg`;
        try {
          const url = await uploadToStorage(accessToken, path, img);
          newSubImages.push(url);
          uploadCount++;
          console.log(`  ✅ 서브 이미지 ${i + 1} 업로드 완료 (${(img.length / 1024).toFixed(0)}KB)`);
        } catch (e) {
        ...
        console.log(`  ⏭️ 서브 이미지 ${i + 1} - 이미 URL`);
      }
    }
...
async function main() {
  console.log('🚀 대용량 스펙북 아이템 마이그레이션 시작\n');
  ...
  for (const itemId of STILL_FAILED_IDS) {
    ...
    await new Promise(r => setTimeout(r, 2000)); // 속도 조절 (대용량이므로 2초)
  }
