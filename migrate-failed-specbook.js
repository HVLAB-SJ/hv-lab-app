/**
 * 실패한 specbook_items를 Firebase Storage로 이미지 분리 후 Firestore에 저장
 * 실패한 아이템: 704NI, 781NI, 스탠리, NEOREST NX 등 19개
 */
...
async function updateFirestoreDoc(accessToken, docId, data) {
  return new Promise((resolve, reject) => {
    const fields = {};
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          reject(new Error('Firestore update failed: ' + res.statusCode + ' ' + data.substring(0, 300)));
        }
      });
    });
    req.on('error', reject);
...
async function processItem(accessToken, item) {
  ...
  if (detail.main_image && isBase64Image(detail.main_image)) {
    ...
    try {
      processedItem.main_image = await uploadToStorage(accessToken, path, detail.main_image, getContentType(detail.main_image));
      imageCount++;
  ...
  if (Array.isArray(detail.sub_images) && detail.sub_images.length > 0) {
    ...
    console.log(`  ✅ 서브 이미지 ${newSubImages.length}개 처리`);
  }

  console.log(`  총 ${imageCount}개 이미지를 Storage로 이전`);

  // Firestore에 저장
  await updateFirestoreDoc(accessToken, String(item.id), processedItem);
...
async function main() {
  console.log('🚀 실패한 specbook_items 마이그레이션 시작\n');
  ...
  for (const item of targetItems) {
    try {
      const imgCount = await processItem(accessToken, item);
