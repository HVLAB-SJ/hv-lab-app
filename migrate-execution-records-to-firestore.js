/**
 * 실행내역 데이터를 Railway SQLite에서 Firestore로 마이그레이션
 */
...
async function getRecordsFromRailway() {
  return new Promise((resolve, reject) => {
    const req = https.request({
...
function transformRecordData(record) {
  // 이미지 처리 - base64 이미지가 너무 크면 제외
  ...
  return {
    ...
    materialCost: record.material_cost || 0,
    laborCost: record.labor_cost || 0,
    vatAmount: record.vat_amount || 0,
    totalAmount: record.total_amount || 0,
    notes: record.notes || '',
    paymentId: record.payment_id,
    includesTaxDeduction: record.includes_tax_deduction === 1,
    includesVAT: record.includes_vat === 1,
    createdAt: record.created_at,
    updatedAt: record.updated_at,
    // 이미지는 크기가 크면 제외
    images: hasLargeImages ? [] : images.slice(0, 3),
...
async function main() {
  console.log('🔄 실행내역 마이그레이션 시작\n');

  // 1. Firestore 토큰 발급
  const firestoreToken = await getFirestoreToken();
  console.log('✅ Firestore 토큰 발급 완료\n');

  // 2. Railway에서 실행내역 목록 가져오기
  const records = await getRecordsFromRailway();
  console.log(`📦 Railway에서 ${records.length}개 실행내역 조회\n`);

  // 3. Firestore에 실행내역 저장
  let created = 0;
  let updated = 0;
  let failed = 0;
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    try {
      const firestoreData = transformRecordData(record);
      const docId = String(record.id);

      const result = await createFirestoreDoc(firestoreToken, 'execution_records', docId, firestoreData);

      if (result === 'exists') {
        await updateFirestoreDoc(firestoreToken, 'execution_records', docId, firestoreData);
        updated++;
      } else {
        created++;
      }

      process.stdout.write(`\r  진행: ${i + 1}/${records.length} (생성: ${created}, 업데이트: ${updated})`);
    } catch (error) {
      failed++;
