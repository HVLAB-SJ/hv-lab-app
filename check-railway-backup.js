...
// Find the railway backup file
const files = fs.readdirSync(__dirname);
const railwayBackup = files.find(f => f.startsWith('railway-backup-') && f.endsWith('.db'));

if (!railwayBackup) {
  console.error('❌ Railway 백업 파일을 찾을 수 없습니다.');
  process.exit(1);
}

const dbPath = `./${railwayBackup}`;
console.log(`=== Railway 백업 데이터베이스 분석 ===`);
