...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-3 md:space-y-5 lg:space-y-4">
      {/* 사람별 할일 섹션 */}
      <div>
        <div className="dashboard-grid grid grid-cols-1 sm:grid-cols-2 ipad:grid-cols-2 ipad-lg:grid-cols-2 ipad-xl:grid-cols-3 ipad-2xl:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4 lg:gap-4">
          {TEAM_MEMBERS.map((member) => {
            ...
            return (
              <div key={member} className={`card p-3 sm:p-4 md:p-5 lg:p-4 ${isCurrentUser ? 'ring-1 ring-gray-300' : ''}`}>
                {/* 헤더 */}
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-lg md:text-lg text-gray-900">{member}</h3>
                    {isCurrentUser && (
                      <span className="text-xs px-2 py-0.5 bg-gray-200 text-gray-600 rounded font-medium">나</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {isCurrentUser && (
